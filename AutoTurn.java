...
public class AutoTurn extends PIDCommand {
  ...
  public AutoTurn(double deg, Drivetrain drivetrain) {
    super(
  ...
  public void initialize() {
    ...
    btnCounter = 0;
    if (RobotContainer.drive.pigeon.setFusedHeading(0).value > 0){
      RobotContainer.drive.printDebug("ERROR ON SETTING HEADING");
    }; 
    ...
    setTimeout(10);
    RobotContainer.drive.brakeMode();
    gyroIsReset = true;
    getPIDController().enable();
    RobotContainer.photon.addAnimation("AutoTurn", Animation.BLINK_DUAL, Color.WHITE, Color.PURPLE, 50, 5);
  }
  ...
  public void execute() {
      if (getPIDController().isEnabled()){
        ...
        RobotContainer.drive.DriverArcadeDrive(0, m_steerValue);
      }
  ...
  public boolean isFinished() {
    ...
    if (gyroIsReset && this.timeSinceInitialized() > 0.2){
      if (Math.abs(this.getSetpoint()) < Math.abs(returnPIDInput())){
        RobotContainer.drive.printDebug("Auto Turn Overshot");
        return true;  //If we over shot, just stop
      }
      if (isTimedOut()){
        RobotContainer.drive.printDebug("Auto Turn Timeout");
        return true;
      }
  ...
  protected void end() {
    ...
    RobotContainer.drive.defaultIdleMode();
    getPIDController().disable();
    setSetpoint(0);                         //Reset the target on exit
    RobotContainer.drive.logEvent("Turn Finished");
  }
  ...
  protected double returnPIDInput(){
      return RobotContainer.drive.pigeon.getFusedHeading();
  };
  ...
  public void updatePrefsPDF(){
    double p = RobotContainer.prefs.getNumber("D: Steer P", 0.1);
    double d = RobotContainer.prefs.getNumber("D: Steer D", 0.0);
    double f = RobotContainer.prefs.getNumber("D: Steer F", 0.0);
    this.setPDF(p, d, f);
  }
